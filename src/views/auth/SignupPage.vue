<template>
  <div>
    <v-card class="text-center pa-1">
      ...
      <v-card-text>
        <v-form ref="form" v-model="isFormValid">
          ...
          <v-btn :loading="isLoading" :disabled="isSignUpDisabled" block size="x-large" color="primary" @click="submit">
            {{ $t("register.button") }}
          </v-btn>
    ...
    <div class="text-center mt-6">
      {{ $t("register.account") }}
      <a class="font-weight-bold cursor-pointer" @click="toLoginModule('sign-in')">
        {{ $t("register.signin") }}
      </a>
    </div>
  </div>
