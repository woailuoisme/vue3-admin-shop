<template>
  <v-card>
    ...
    <v-data-table :headers="headers" :items="items" hide-default-footer>
      ...
      <template #[`item.date`]="{ item: { raw } }">
        <div>{{ $filter.formatCurrency(raw.date) }}</div>
      </template>

      <template #[`item.company`]="{ item: { raw } }">
        <copy-label :text="raw.company" />
      </template>

      <template #[`item.amount`]="{ item: { raw } }">
        {{ $filter.formatCurrency(raw.amount) }}
      </template>

      <template #[`item.status`]="{ item: { raw } }">
