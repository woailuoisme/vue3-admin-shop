<template>
  <v-card>
    <v-card-title>
      <span class="text-h5 text-primary">{{ isNew ? "新增" : "编辑" }}</span>
    </v-card-title>
    <v-card-text>
      <v-container>
              ...
              <v-hover v-slot="{ hover }">
                <v-card class="mx-auto" color="grey lighten-4" max-width="600">
                  <v-row align="center" justify="center">
                    <v-img :src="item.image" class="d-flex mt-3" :aspect-ratio="6 / 5" @error="item.image = dImgOne">
    ...
    <v-card-actions>
      <v-spacer />
      <v-btn variant="outlined" color="info" text @click="close">取消</v-btn>
      <v-btn variant="elevated" :disabled="!valid" color="primary" @click="save">保存</v-btn>
    </v-card-actions>
  </v-card>
</template>
