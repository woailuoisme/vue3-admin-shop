      ...
      <v-container>
        <v-form v-model="valid">
          ...
          <v-row>
            ...
            <v-col cols="3">
              <v-switch color="primary" v-model="editedItem.is_sale" inset
                        :label="`是否上线？答：${saleLabel(editedItem.is_sale)}`"/>
            </v-col>

            <v-col cols="3">
              <v-text-field v-model="editedItem.sale_price" label="销售价格" :rules="[rule.required('销售价格')]" type="number"
                            step="0.01"/>
            </v-col>

            <v-col cols="3">
              <v-text-field v-model="editedItem.stock" label="库存" type="number" :rules="[rule.required('库存')]"
                            step="1"/>
            </v-col>
          </v-row>
          <v-row>
            <v-col cols="12">
              ...
              <my-editor v-model="editedItem.content" :rules="[rule.required('产品内容')]"
                         :imgUploadUrl="`http://localhost/`"/>
            </v-col>
          </v-row>
          <v-row>
            <v-col cols="12">
              ...
              <v-hover v-slot="{ hover }">
                <v-card class="mx-auto" color="grey lighten-4" max-width="600">
                  <v-row align="center" justify="center">
                    <v-img :src="item.image" class="d-flex mt-3" :aspect-ratio="6 / 5"
                           @error="item.image = defaultImage">
