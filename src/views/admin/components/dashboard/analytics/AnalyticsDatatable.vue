...
const headers = ['NAME', 'EMAIL', 'DATE', 'SALARY', 'AGE', 'STATUS']
const usreList = data
</script>

<template>
  <v-card>
    <v-table :headers="headers" :items="usreList" item-key="fullName" class="table-rounded" hide-default-footer disable-sort>
      <thead>
        <tr>
      ...
      <tbody>
        <tr v-for="row in data" :key="row.fullName">
          <!-- name -->
          ...
          <!-- status -->
          <td>
            <v-chip small :color="statusColor[status[row.status]]" class="font-weight-medium">
              {{ status[row.status] }}
            </v-chip>
          </td>
        </tr>
